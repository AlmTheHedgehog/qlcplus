            ...
            case Tardis::NetAuthentication:
            {
                ...
                if (success == true)
                {
                    ...
                    if (m_autostartServer)
                    {
                        ...
                        if (sendWorkspaceToClient(host->hostName, "NewProject.autosave.qxw")) // TODO: have to provide filename of current session as 2nd client will reset the session. But the session file is present in App - use signals to retrieve it from there?
